...
def svrg_ht( f, regularizer, epoch, batch_size , stepsize= 10**-5,  stepsize_type = "fixed", verbose = False, optgap = 10**(-3 ), loss = 'logistic',ht_k=100,log_interval=1,output_folder='../logs/', multi_class= False):
  '''
  INPUT:
  x : data
  ...
  y = y[index ]
  #label = (label == 1 )* 1
  folder_name = os.path.basename(f) + '_loss_' + str(loss)  +'_opt_svrg_'+  'stepsize_'+str( stepsize)+'_batchsize_'+ str(batch_size)+'_ht_k_'+ str(ht_k)+'_log_interval_'+ str(log_interval)+'_epoch_'+ str(epoch)
  ...
  for k in range (int(epoch/2)):
    ...
    #--------------------------------------
      ...
      if (t+1) % log_interval == 0:
          ...
          logger.add_scalar('loss_number-IFO', obj_temp, (2*k)*x.shape[0] + 2*t*batch_size )
          logger.add_scalar('loss_number-HT', obj_temp, iteration)
          file.write(str( ((2*k)*x.shape[0] + 2*t*batch_size)/x.shape[0])+','+str(iteration)+','+ str(obj_temp)+'\n')  
  file.close()
  
def main(batch_size = 1, eta=0.01, epoch = 10):
  #f = sys.argv[1]
  #dataset = 'real-sim'
  f= 'F:/Users/gul15103/Desktop/simulate_data1000_2500_0.5.npz'
  ...
  svrg_ht( f, regularizer, epoch, batch_size,  stepsize= eta, stepsize_type = "fixed", verbose = True, optgap = 10**(-30), loss = loss ,ht_k=200, multi_class= True)
